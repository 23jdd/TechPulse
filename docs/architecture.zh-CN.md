...
TechPulse 是一个 Go monorepo。主要服务入口都放在 `cmd/` 目录下，可以独立运行。
...
Phase 1 在 gateway 进程内跑通真实 MVP 路径。Phase 2 将 scheduler、fetcher、parser、AI pipeline、search、RAG、worker 暴露为可独立运行的服务。
...
- Redis：缓存热点 REST 响应和服务状态。Redis 不可用时 gateway 会自动回退到 MySQL/Bleve
